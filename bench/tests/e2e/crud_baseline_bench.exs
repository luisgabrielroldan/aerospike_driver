...
defmodule Aerospike.Bench.E2E.CrudBaseline do
  ...
  defp build_state(namespace, set, small_payload, large_payload) do
    %{
      ...
      small_read_key: Aerospike.key(namespace, set, "e2e:read:small"),
      large_read_key: Aerospike.key(namespace, set, "e2e:read:large"),
      small_ring:
        namespace
  ...
  defp bootstrap_dataset!(conn_name, state) do
    :ok = Aerospike.put(conn_name, state.small_read_key, state.small_payload)
    :ok = Aerospike.put(conn_name, state.large_read_key, state.large_payload)
