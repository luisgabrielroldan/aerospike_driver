defmodule Aerospike.Node do
  @moduledoc """
  Per-node struct plus pure (given a transport) info-socket operations for
  one cluster member.

  A `%Aerospike.Node{}` captures the observable facts the tend cycle reads
  ...
  `Aerospike.Tender`'s node map; the functions here take and return a
  `%Node{}` struct explicitly so the caller remains in control of when
  state transitions are committed. I/O runs through the
  `Aerospike.NodeTransport` behaviour module supplied as an argument — the
  ...
  @typedoc """
  Cached login session returned by `Aerospike.NodeTransport.login/2`:
  ...
  def parse_features(node_name, info) do
    case Map.fetch(info, "features") do
      ...
      _ ->
        Logger.debug(fn ->
          "Aerospike.Node: #{node_name} features key absent; assuming none"
        end)
  ...
  def fetch_features(transport, conn, node_name) do
    case transport.info(conn, ["features"]) do
      ...
      {:error, %Error{} = err} ->
        Logger.debug(fn ->
          "Aerospike.Node: peer #{node_name} features probe failed: #{err.message}"
        end)
