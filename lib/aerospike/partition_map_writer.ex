defmodule Aerospike.PartitionMapWriter do
  @moduledoc """
  ...
  handler forwards them to `Aerospike.PartitionMap` (for `put_node_gen`,
  `drop_node`, `delete_node_gen`) or `Aerospike.PartitionMapMerge` (for
  ...
  The writer does not own the ETS tables. `Aerospike.TableOwner` creates
  ...
  `Aerospike.Tender.tend_now/1` sees a fully-committed cycle: when the
  ...
  via `Aerospike.PartitionMapMerge.apply_segments/4`.
  ...
  def init(opts) do
    ...
    is_atom(name) or
      raise ArgumentError, "Aerospike.PartitionMapWriter: :name must be an atom"
