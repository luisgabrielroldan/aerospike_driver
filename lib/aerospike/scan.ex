defmodule Aerospike.Scan do
  @moduledoc """
  Composable scan description (pure data, no I/O).

  Build a scan with `new/1` or `new/2`, then chain `select/2`, `filter/2`,
  `max_records/2`, and related functions. Execution is performed by the client
  facade in a later phase.

  Expression filters (`filter/2`) are combined with **AND** semantics at execution time.
  """
  ...
  @doc """
  Starts a namespace-wide scan (no set filter).
  """
  @spec new(String.t()) :: t()
  ...
  @doc """
  Starts a scan limited to a single set.
  """
  ...
  def filter(%__MODULE__{} = scan, %Exp{} = exp) do
    %{scan | filters: scan.filters ++ [exp]}
  end
  ...
  @doc """
  Attaches a partition filter for partial scans or resume.
  """
  @spec partition_filter(t(), PartitionFilter.t()) :: t()
  def partition_filter(%__MODULE__{} = scan, %PartitionFilter{} = pf) do
    %{scan | partition_filter: pf}
  end

  @doc """
  When `true`, the server omits bin payloads (useful for count-style scans).
  """
