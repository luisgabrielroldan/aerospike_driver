defmodule AeroMarketLive.MarketReader do
  ...
  @doc """
  Fetches per-day summaries for a ticker over the given days in a single
  batch round-trip.
  ...
  def fetch_day_range(conn, ticker, day_list)
    ...
    batch_ops =
      Enum.map(day_list, fn day_ms ->
        key = Aerospike.key(@namespace, @timeseries_set, "#{ticker}#{day_ms}")
        Batch.operate(key, day_ops(), respond_per_each_op: true)
      end)

    case Aerospike.batch_operate(conn, batch_ops) do
      {:ok, results} ->
        results
        |> Enum.zip(day_list)
        |> Enum.flat_map(fn
          {%{status: :ok, record: record}, day_ms} ->
            case build_summary(record, ticker, day_ms) do
              {:ok, summary} -> [summary]
              :empty -> []
            end
      ...
      {:error, reason} ->
        Logger.warning(
          "[MarketReader] fetch_day_range batch failed for #{ticker}: #{inspect(reason)}"
        )
