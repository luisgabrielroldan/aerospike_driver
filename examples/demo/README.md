...
From the `aerospike_driver` directory, start the localhost stack needed for
all demo checks (community, enterprise, TLS, and mTLS):
...
- `Demo.SecurityRepo` (`name: :aero_security`) -> `localhost:3200`

`Demo.Application` starts the primary and enterprise cluster supervisors at
boot:
...
The TLS examples connect directly with `Aerospike.Transport.Tls` so that the
mTLS example can provide client certificates only when it runs.
...
| `QueryBackground` | Background query jobs with `query_execute`, `query_udf`, and `ExecuteTask.wait` |
...
| `RuntimeMetrics` | Runtime metrics lifecycle and explicit pool warm-up |
| `NodeTargeted` | Node-targeted scan count and secondary-index query with `node: node_name` |
...
| `SecurityAdmin` | Requires local Enterprise security endpoint on `localhost:3200` |
| `XdrFilter` | Requires an Enterprise endpoint that accepts XDR filter commands; skips when unavailable |
