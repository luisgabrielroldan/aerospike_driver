defmodule Aerospike.Protocol.MessagePack do
  ...
  defp pack_iolist(n) when is_integer(n) do
    cond do
      n >= -32 and n <= -1 -> [<<256 + n::8>>]
      n >= -128 -> [<<0xD0, n::8-signed>>]
      n >= -0x8000 -> [<<0xD1, n::16-signed-big>>]
      n >= -0x8000_0000 -> [<<0xD2, n::32-signed-big>>]
      true -> [<<0xD3, n::64-signed-big>>]
    end
  end
  ...
  defp do_unpack(<<b, rest::binary>>) when b >= 0xE0, do: {:ok, {b - 256, rest}}
  ...
  defp do_unpack(<<0xC7, len::8, type::8, data::binary-size(len), rest::binary>>),
    do: {:ok, {{:ext, type, data}, rest}}

  defp do_unpack(<<0xC8, len::16-big, type::8, data::binary-size(len), rest::binary>>),
    do: {:ok, {{:ext, type, data}, rest}}

  defp do_unpack(<<0xC9, len::32-big, type::8, data::binary-size(len), rest::binary>>),
    do: {:ok, {{:ext, type, data}, rest}}

  defp do_unpack(_), do: {:error, :invalid_msgpack}

  defp unpack_list(binary, 0, acc), do: {:ok, {Enum.reverse(acc), binary}}

  defp unpack_list(binary, count, acc) do
    case do_unpack(binary) do
      {:ok, {term, rest}} -> unpack_list(rest, count - 1, [term | acc])
      {:error, _} = error -> error
    end
  end

  defp unpack_map(binary, 0, acc), do: {:ok, {acc, binary}}

  defp unpack_map(binary, count, acc) do
    with {:ok, {key, rest1}} <- do_unpack(binary),
         {:ok, {value, rest2}} <- do_unpack(rest1) do
      unpack_map(rest2, count - 1, Map.put(acc, key, value))
    end
  end

  defp unpack_cdt_string_payload(<<@particle_string, utf8::binary>>), do: utf8
  defp unpack_cdt_string_payload(other), do: other
end
