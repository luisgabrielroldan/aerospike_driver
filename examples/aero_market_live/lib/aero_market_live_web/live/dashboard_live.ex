defmodule AeroMarketLiveWeb.DashboardLive do
  ...
  def mount(_params, _session, socket) do
    ...
    socket =
      assign(socket,
        ...
        sparklines: Map.new(@symbols, fn sym -> {sym, Sparkline.render([])} end),
  ...
  def handle_info({:tick, sym, day_ms, _index, price}, socket)
    ...
    socket =
      if day_ms != socket.assigns.today_ms do
        ...
        assign(socket,
          ...
          sparklines: Map.new(seeded_bufs, fn {s, buf} -> {s, Sparkline.render(buf)} end),
    ...
    updated_buf =
      ...
      |> Map.fetch!(sym)
      |> Sparkline.prepend(price, @ring_size)

    {:noreply,
     assign(socket,
       tick_bufs: Map.put(socket.assigns.tick_bufs, sym, updated_buf),
       sparklines: Map.put(socket.assigns.sparklines, sym, Sparkline.render(updated_buf)),
  ...
  defp fetch_initial_ticks(conn, symbols, day_ms) do
    Map.new(symbols, fn sym ->
      key = Aero.key(@namespace, @timeseries_set, "#{sym}#{day_ms}")
      ops = [MapOp.get_by_index_range("stock", -@ring_size, @ring_size)]

      prices =
        case Aerospike.operate(conn, key, ops) do
          {:ok, record} -> Sparkline.from_cdt_map(record.bins["stock"], @ring_size)
          {:error, _} -> []
        end
