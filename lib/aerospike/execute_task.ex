defmodule Aerospike.ExecuteTask do
  ...
  defp fetch_node_status(task, node_name) do
    ...
    with {:ok, pool_pid, ^node_name} <- Router.node_pool(task.conn, node_name),
         {:ok, map} <-
  ...
  defp target_node_names(%__MODULE__{conn: conn, node_name: node_name})
    ...
    if cluster_ready?(conn) do
      case :ets.lookup(Tables.nodes(conn), node_name) do
        [{^node_name, _row}] -> {:ok, [node_name]}
  ...
  defp cluster_ready?(conn) do
    match?([{_, true}], :ets.lookup(Tables.meta(conn), Tables.ready_key()))
  end
