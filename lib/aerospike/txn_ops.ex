defmodule Aerospike.TxnOps do
  @moduledoc false
  # Internal ETS-backed transaction state tracker.
  #
  # Stores per-transaction tracking data in the `txn_tracking` ETS table:
  ...
  def init_tracking(conn_name, %Txn{} = txn) do
    entry = {txn.id, empty_tracking()}
    :ets.insert(Tables.txn_tracking(conn_name), entry)
  end
  ...
  def track_read(conn_name, %Txn{} = txn, %Key{} = key, version) do
    if version != nil do
      update_tracking(conn_name, txn, fn t ->
        %{t | reads: Map.put(t.reads, key, version)}
      end)
    else
      true
    end
  ...
  def cleanup(conn_name, %Txn{} = txn) do
    :ets.delete(Tables.txn_tracking(conn_name), txn.id)
  end
  ...
  defp update_tracking(conn_name, %Txn{} = txn, fun) do
    case :ets.lookup(Tables.txn_tracking(conn_name), txn.id) do
      [{_, data}] ->
        :ets.insert(Tables.txn_tracking(conn_name), {txn.id, fun.(data)})
