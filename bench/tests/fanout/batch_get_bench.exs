...
defmodule Aerospike.Bench.Fanout.BatchGet do
  ...
  def run do
    ...
    target = Runtime.connection_target("bench_fanout")

    batch_sizes = Map.get(config, :fanout_batch_sizes, [@default_batch_size])
    ...
    E2EHelpers.with_started_client(
      ...
      fn ->
        ...
        Runtime.print_metadata(metadata, %{workload: :fo_001})
  ...
  defp build_state(namespace, set, payload, batch_sizes) do
    batch_keysets =
      Enum.map(batch_sizes, fn batch_size ->
        keys = E2EHelpers.key_ring(namespace, set, "fo:batch:get:#{batch_size}", batch_size)
        {batch_size, keys}
      end)
